...
data class MontageScore(
    ...
    /** Deux plans voisins ne viennent pas du même moment de la même partie. */
    val variety: Double,
    /** Durée demandée réellement utilisée. */
    val fill: Double,
    /** Les plans sont-ils plus courts dans les sections intenses ? */
    val pacing: Double,
...
object MontageScorer {
    ...
    fun score(plan: MontagePlan): MontageScore {
        ...
        val variety = if (pairs.isEmpty()) 1.0 else 1.0 - similar.toDouble() / pairs.size

        // --- remplissage de la durée demandée.
        val fill = (plan.duration / plan.settings.maxDuration).coerceIn(0.0, 1.0)

        // --- rythme : à chaque paire de plans d'intensités différentes, le plus intense doit être le plus court.
        var comparable = 0
        var concordant = 0
        for (i in plan.clips.indices) {
            for (j in i + 1 until plan.clips.size) {
                val a = plan.clips[i]
                val b = plan.clips[j]
        ...
        val pacing = if (comparable == 0) 1.0 else concordant.toDouble() / comparable
        ...
        val total = WEIGHTS.entries.sumOf { (k, w) -> w * parts.getValue(k) }
        return MontageScore(
